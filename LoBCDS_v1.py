...
sel_bck_node = -1               #Select nodes to add to the backbone
sel_node = -1                   #Select node to search for nodes in its 1-Nbr
counter_1 = 0 
counter_2 = 0 
bck_node_limit = num_Nodes      #Max number of nodes to be added to existing backbone 
LF_val_limit = 3                #The algorithm exits after 3 fail attempts to minimize MAX-LF 
color_flag = 0
    ...
    for node in nbr1[sel_node]:
        if(CDS[node] != 1 and counter_1 < bck_node_limit):
            ...
            for m in range(Num_Nodes):
                ...
                FY_Nodes_t.append(FY_Nodes[m])
                    
            Nodes_t.append(node)
            FY_Nodes_t.append(node)
...
print "Max LoadFactor :%f" %(LoadFactor.max())
print "Min LoadFactor :%f" %(LoadFactor.min())
print "Avg LoadFactor :%f" %(LoadFactor.sum()/Num_Nodes)
print Load        
