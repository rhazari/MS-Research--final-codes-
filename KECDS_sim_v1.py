# -*- coding: utf-8 -*-
"""
Created on Thu Sep 26 14:16:51 2013
@author: ryan
"""
...
OneHopRange = np.linspace(1.5,4.5,16)

#K connected factor
K = 2
#Number of Nodes in each partition
num_nodes_prt = num_Nodes/K
...
for a in range(count):
    ...
    for b in range(test_runs):
        ...
        ev_degree = np.zeros(num_nodes_prt)
        for node in range(num_nodes_prt):
        ...
        ev_nbr = []
        for m in range(num_nodes_prt):
            ev_nbr.append(ev_nbr1[m]+ev_nbr2[m])
        
        ev_CDS = cf.ECDS(ev_Nodes,ev_degree,ev_nbr1,ev_nbr)
        ...
        #Degree of each Node(i.e the no. of 1 Hop neighbors)
        od_degree = np.zeros(num_nodes_prt)
        for node in range(num_nodes_prt):
        ...
        od_CDS = cf.ECDS(od_Nodes,od_degree,od_nbr1,od_nbr)
        '''--------Partition 2--------'''

        #2-connected Dominant Nodes
        ...
        #Computing the Load Factor for the Dominant Nodes        
