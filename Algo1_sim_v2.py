# -*- coding: utf-8 -*-
"""
Created on Tue Sep 10 00:06:32 2013
@author: ryan
"""
...
OneHopRange = np.linspace(2.1,4.6,11)
...
for a in range(count):
    ...
    for b in range(test_runs):
        ...
        read = 1; sel_node = -1; maxLF = 0; color_flag = 0 
        sel_bck_node = -1; counter_1 = 0; counter_2 = 0
        bck_node_limit = 4; LF_val_limit = 3
        while(read):
            ...
            for node in nbr1[sel_node]:
                if(CDS[node] != 1 and counter_1 < bck_node_limit):
                    ...
                    Nodes_t.append(node)
                    Nbr1_t = []; Nbr1_t = OneHopNbr(Nodes_t,nbr_range)
...
for m in range(count):
    my_file.write(str(percent_node_contention[m])+"\t")
my_file.close()